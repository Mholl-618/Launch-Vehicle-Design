function [t,dx,masses,q,h_mag] = throttling_launch(t,x,masses,del_t,hyp_perc)
...
if x(2) < 10e3 %before grav turn alt
    fpa = 1; %go straight up
    var_pass = 0;
else %grav turn profile
    var_pass = (tanh(hyp_val-(hyp_val*((150e3-x(2))/140e3))))*(pi/2); %final alt of 150
    fpa = cos(var_pass);
end
